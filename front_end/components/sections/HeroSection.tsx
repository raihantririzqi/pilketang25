...
import { breathingAnimation, containerVariants, fadeInUpVariants, floatingAnimation, popUpVariants, slowFloatingAnimation } from "../common/animation";
        ...
        <section id="hero-section" className="flex flex-col h-screen container mx-auto py-10 items-center justify-evenly">
            ...
            >
                <div className="flex flex-col items-center gap-8 md:grid md:grid-cols-2 md:gap-32">
                    {/* LOGO BESAR */}
                    {/* --- LOGO BESAR DENGAN ANIMASI IDLE --- */}
                    {/* 1. Wrapper Luar: Animasi Muncul (Pop Up) */}
                    <motion.div
                        variants={popUpVariants}
                        className="bg-white rounded-full h-52 w-52 md:h-80 md:w-80 p-4 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)]"
                    >
                        {/* 2. Wrapper Dalam: Animasi Idle (Slow Floating) */}
                        <motion.div
                            animate={slowFloatingAnimation}
                            className="w-full h-full" // Pastikan wrapper dalam mengisi penuh
                        >
                            <Image
                                ...
                                width={300}
                                height={150}
                                className="object-contain" // Opsional: menjaga rasio gambar
                            />
                        </motion.div>
                    </motion.div>
                    ...
                    <div className="flex flex-col gap-8 items-center">
                        ...
                        {/* --- 1. DEKORASI BINTANG & BADGE (Efek Melayang) --- */}
                        {/* motion.div LUAR menangani animasi MUNCUL (PopUp) */}
                        <motion.div variants={popUpVariants} className="relative -rotate-6">

                            {/* motion.div DALAM menangani animasi IDLE (Floating) */}
                        ...
                        {/* --- 2. TEKS JUDUL (Efek Bernapas Halus) --- */}
                        {/* motion.div LUAR menangani animasi MUNCUL (FadeInUp) */}
                        <motion.div
                            variants={fadeInUpVariants}
                            className="flex flex-col font-roster text-4xl md:text-6xl font-bold text-center leading-snug"
                        >
                            {/* motion.div DALAM menangani animasi IDLE (Breathing) */}
                            <motion.div animate={breathingAnimation}>
                                <div>Selamat</div>
                                <div>Datang Di</div>
                                <div className="text-magenta drop-shadow-md">Peduli Hati</div>
                            </motion.div>
                        </motion.div>
